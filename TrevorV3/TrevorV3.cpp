...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_COMMAND: {
        if (LOWORD(wParam) == 1) {
            ...
            if (!IsRunAsAdmin()) {
                MessageBoxW(hwnd, L"Trevor Injector v3 MUST be run as Administrator.", L"Error", MB_OK | MB_ICONERROR);
                PlayErrorSound();
...
int WINAPI wWinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPWSTR lpCmdLine, _In_ int nCmdShow) {
    if (!IsRunAsAdmin()) {
        MessageBoxW(NULL, L"Trevor Injector v3 MUST be run as Administrator.", L"Error", MB_OK | MB_ICONERROR);
        return -1;
    }
